            ...
            public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
                ...
                switch (index){
                    case 0://start
                        ...
                        start.setText("Start:"+start_year + "-" + start_month + "-" + start_day);
                        break;
                    case 1://stop
                        stop_year = year;
                        stop_month = monthOfYear + 1;
                        stop_day = dayOfMonth;
                        stop.setText("Finish:"+stop_year + "-" + stop_month + "-" + stop_day);
                        break;
                    default:
                        break;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.plan_start://设置开始日期
                index = 0;
                datePickerDialog.setTitle("Set the start time");
                datePickerDialog.show();
                break;
            case R.id.plan_stop://设置结束日期
                index = 1;
                datePickerDialog.setTitle("Set end time");
                datePickerDialog.show();
                break;
...
//                    Log.e("执行了？","加上了24个小时");
                ...
                if (DateUtils.getMillisecondValues(start_year,start_month,start_day) > DateUtils.getMillisecondValues(stop_year,stop_month,stop_day)){
                    Toast.makeText(this,"The start time cannot be greater than the end time!",Toast.LENGTH_SHORT).show();
                    return;
                }
                if (DateUtils.getMillisecondValues((Integer) DateUtils.getDate().get("year"),(int) DateUtils.getDate().get("month"),(int) DateUtils.getDate().get("day")) > DateUtils.getMillisecondValues(stop_year,stop_month,stop_day)){
                    Toast.makeText(this,"The end time cannot be less than the current time!",Toast.LENGTH_SHORT).show();
                    return;
                }
...
//                    Log.e("查询出的相同数据ID", selectID + "");
                    if (selectID != 0){
                        ...
                        dialog.setMessage("The time point you want to change is already occupied. Do you want to overwrite the exercise plan for that time point?");
    ...
    private void insertData(ContentValues values){
        if (isToSave){
            ...
            if (result > 0){
                Toast.makeText(this,"Successfully set up the plan!",Toast.LENGTH_SHORT).show();
                setResult(RESULT_OK);
                finish();
            }else {
                Toast.makeText(this,"Failed to set the plan! Please set the plan again!",Toast.LENGTH_SHORT).show();
            }
        }
    }
    /**
